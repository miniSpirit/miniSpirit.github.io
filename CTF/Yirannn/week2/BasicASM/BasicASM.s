...
->00007FF7A8AC5AB4  movsxd      rax,dword ptr [v1]  
rbp+64H扔到rbp+1F8H,在这里我们不妨定义64H这里是v1, 1F8H是v2, v1应该是个循环变量
00007FF7A8AC5AB8  mov         qword ptr v2,rax (1F8H) 
又扔一个Flag到rcx里
00007FF7A8AC5ABF  lea         rcx,[flag]  
猜测每次这个basic_string操作的参数都是rcx，
虽然本地编译一个cpp的时候不是这种情况，但猜测应该是不同系统或者编译器的不同行为

读取长度并扔到rax里面
00007FF7A8AC5AC3  call        std::basic_string<char,std::char_traits<char>,std::allocator<char> >::length (07FF7A8AC122Bh)  
00007FF7A8AC5AC8  mov         rcx,qword ptr v2  
00007FF7A8AC5ACF  cmp         rcx,rax  
00007FF7A8AC5AD2  jae         main+1B2h (07FF7A8AC5C02h)  ; 如果rcx >= rax, 跳转104行
00007FF7A8AC5AD8  mov         eax,dword ptr [v1]  v1扔到eax里面
00007FF7A8AC5ADB  and         eax,1  
00007FF7A8AC5ADE  cmp         eax,1  如果v1是奇数，则继续，否则跳到B76
00007FF7A8AC5AE1  jne         main+126h (07FF7A8AC5B76h)  
00007FF7A8AC5AE7  movsxd      rax,dword ptr [v1]  
...
00007FF7A8AC5B0E  mov         qword ptr v2,rax  
...
输出，前面设置了输出宽度(2)，十六进制，填充之类的，反正就是cout的参数之类的
...
00007FF7A8AC5B53  mov         rcx,qword ptr v2  
...
00007FF7A8AC5B71  jmp         main+1ADh (07FF7A8AC5BFDh)  main+1AD会跳回到AAC

00007FF7A8AC5B76  movsxd      rax,dword ptr [rbp+64h]  ; 偶数跳到这里
...
00007FF7A8AC5B9A  mov         qword ptr v2,rax  
...
00007FF7A8AC5BDF  mov         rcx,qword ptr v2  
...
00007FF7A8AC5BFD  jmp         main+5Ch (07FF7A8AC5AACh)  跳回循环开头
00007FF7A8AC5C02  mov         dword ptr [rbp+1E4h],0  ;40行会跳到这里
